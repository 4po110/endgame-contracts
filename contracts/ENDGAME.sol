...
contract EndGame is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 ENDGAME-WFTM LP -> ENDGAME pool
    uint256 public constant INITIAL_ENDGAME_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the ENDGAME ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("ENDGAME", "ENDGAME") {
        // Mints 1 ENDGAME to contract creator for initial pool setup
    ...
    function _getEndGamePrice() internal view returns (uint256 _endgamePrice) {
        try IOracle(endgameOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("EndGame: failed to fetch ENDGAME price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _endgamePrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_endgamePrice >= taxTiersTwaps[tierId]) {
    ...
    function setEndGameOracle(address _endgameOracle) public onlyOperatorOrTaxOffice {
        require(_endgameOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints ENDGAME to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of ENDGAME to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentEndGamePrice = _getEndGamePrice();
            currentTaxRate = _updateTaxRate(currentEndGamePrice);
            if (currentEndGamePrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        address _endgamePool,
        address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        require(_endgamePool != address(0), "!_endgamePool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_endgamePool, INITIAL_ENDGAME_POOL_DISTRIBUTION);
